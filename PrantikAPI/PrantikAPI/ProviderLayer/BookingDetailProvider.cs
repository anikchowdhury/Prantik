...
namespace PrantikAPI.ProviderLayer
{
    public class BookingDetailProvider
    {
        ...
        internal IEnumerable<BookingDetailModel> GetBookingDetails()
        {
            using (PrantikEntities db = new PrantikEntities())
            {
                var dayBeforeToSearch = DateTime.Today.AddDays(-60);
                return db.BookingDetails.Where(x => x.CreateDate >= dayBeforeToSearch).Include(x => x.BookingDetailsUsers.Select(y => y.User)).Include(x => x.RoomBookings).Include(x => x.OrderDetails.Select(y => y.Menu)).Include(x => x.BookingDetailsPayments.Select(y => y.Payment)).OrderByDescending(x => x.CreateDate).ToList<BookingDetail>().Select(bookingDetail => new BookingDetailModel()
                {
                    Id = bookingDetail.Id,
                    BookingCode = bookingDetail.BookingCode,
                    Users = bookingDetail.BookingDetailsUsers.Select(x => new UserModel()
